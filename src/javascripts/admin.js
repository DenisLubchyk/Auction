$(document).ready(function() {
    ...
    function startAuction() {
        socket.json.emit('startauction', {time: research_time, msg: 'До начала торга: '});
    ...
    function auctionStep() {
        ...
        refresh(timeout);
        $('#timerLabel').text("Идет торг. Окончание через:")
    }

    function researchStep() {
        socket.json.emit('researchstep', {msg: 'Ознакомление'});
        socket.json.emit('refreshtimer', {time: research_time});
        refresh(research_time);
        $('#timerLabel').text("Пауза на изучение. Старт через:")
    }
    ...
    function stopAuction() {
        ...
        socket.json.emit('admin', {value: "Аукцион окончен"});
        $('#timerLabel').text("Аукцион окончен");
        $('#timer').hide();
        $('#timeLabel').hide();
        $("#time").hide();
    }
    ...
    $('#start').on('click', function () {
        startAuction();
        socket.json.emit('admin', {value: "Аукцион начинается"});
        $('#start').hide();
        $('#timerLabel').text("Пауза на изучение. Старт через:");
        $('#timeLabel').text("C начала аукциона прошло:");
        var m = 0;
        var i = 1;
